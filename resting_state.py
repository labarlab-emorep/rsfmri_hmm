...
color_seqs = {}
for subj_id, seq in emo_seqs.items():
    # Map the numbers to colors using the color map
    color_seqs[subj_id] = [color_map.get(num, 'black') for num in seq]

# Create concatenated color sequence
total_color_seq = []
for num in total_scores['label_max']:
    total_color_seq.append(color_map.get(num))

# Plot individual color sequences
""""
fig, ax = plt.subplots(figsize=(10, 10))
gap = 1  # Gap between each subject's color sequence

first_10_subjects = list(color_seqs.items())[:10]
...
for j, color in enumerate(total_color_seq):
...
ax.set_xlim(0, len(total_color_seq))  # Set the x-axis limit based on the length of the color sequence
...
# Get total TM
adj_emo_seqs = {key: [value - 1 for value in values if value > 0] for key, values in emo_seqs.items()}
tm = transition_matrix(adj_emo_seqs.values())

# Get individual TM's for first 10 subjects
tms = {}
for subj_id, seq in list(emo_seqs.items()):
    adj_seq = [s - 1 for s in seq if s > 0]
    tms[subj_id] = transition_matrix([adj_seq])

# Create a mask to threshold transition probabilities
threshold = 0.05
masked_matrix = np.ma.masked_where(tm < threshold, tm)
